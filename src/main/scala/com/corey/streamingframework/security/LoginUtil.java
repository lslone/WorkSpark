...
public class LoginUtil {
    ...
    private static final String LOGIN_FAILED_CAUSE_TIME_WRONG = "(check skew time of local server and remote server not match, please check ntp to remote server)";
    ...
    private static boolean checkNeedLogin(String principal) throws IOException {
        if (!UserGroupInformation.isSecurityEnabled()) {
            LOG.error("UserGroupInformation is not SecurityEnabled,please check if core_site.xml exists in classpath.");
            throw new IOException("UserGroupInformation is not SecurityEnabled,please check if core_site.xml exists in classpath.");
        }
        ...
        if(currentUser !=null && (currentUser.hasKerberosCredentials())){
            if(checkCurrentUserCorrect(principal)){
                ...
                LOG.error("current user is " + currentUser + " had logined.please check your enviroment , especially when it used IBM JDK or kerberos for OS count login!!!");
                throw new IOException("current user is " + currentUser + " had logined.please check your enviroment .");
            }
        }
        return true;
    }
